/*!
 * @nuxt/builder v2.14.3 (c) 2016-2020
...
function template(string, options, guard) {
  ...
  var sourceURL = hasOwnProperty$c.call(options, 'sourceURL')
    ? ('//# sourceURL=' +
...
class Builder {
  ...
  async build () {
    ...
    if (this.options.dev) {
      ...
      const target = utils.isFullStatic(this.options) ? 'full static' : this.options.target;
      consola__default['default'].info(`Target: ${chalk__default['default'].bold.cyan(target)}`);
    }
  ...
  async validatePages () {
    ...
    if (
      !this._nuxtPages ||
      await fsExtra__default['default'].exists(path__default['default'].join(this.options.srcDir, this.options.dir.pages))
    ) {
      return
    }
  ...
  validateTemplate () {
    ...
    for (const depName in templateDependencies) {
      ...
      const pkg = this.nuxt.resolver.requireModule(path__default['default'].join(depName, 'package.json'));
      if (pkg) {
        const validVersion = semver__default['default'].satisfies(pkg.version, depVersion);
        if (!validVersion) {
          consola__default['default'].warn(`${depName}@${depVersion} is recommended but ${depName}@${pkg.version} is installed!`);
        }
      } else {
        consola__default['default'].warn(`${depName}@${depVersion} is required but not installed!`);
      }
    }
  }
  ...
  watchClient () {
    ...
    ]).map(upath__default['default'].normalizeSafe);
