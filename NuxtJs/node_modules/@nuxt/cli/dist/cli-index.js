...
const localNodeModules = path__default['default'].resolve(process.cwd(), 'node_modules');

// Prefer importing modules from local node_modules (for NPX and global bin)
async function _import (modulePath) {
  for (const mp of [
    path__default['default'].resolve(localNodeModules, modulePath),
    modulePath
  ]) {
    try {
      return await Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(mp)); })
    } catch (e) {
