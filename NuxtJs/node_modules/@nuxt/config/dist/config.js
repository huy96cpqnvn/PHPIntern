/*!
 * @nuxt/config v2.14.3 (c) 2016-2020
...
const esm__default = /*#__PURE__*/_interopDefaultLegacy(esm);
...
function getNuxtConfig (_options) {
  ...
  defaultsDeep_1(options, modePreset || options.modes[utils.MODES.universal]);
  ...
  if (!staticAssets.base) {
    const publicPath = utils.isUrl(options.build.publicPath) ? '' : options.build.publicPath; // "/_nuxt" or custom CDN URL
    staticAssets.base = utils.urlJoin(publicPath, staticAssets.dir);
  }
  if (!staticAssets.versionBase) {
    staticAssets.versionBase = utils.urlJoin(staticAssets.base, staticAssets.version);
  }
...
async function loadNuxtConfig ({
  ...
  configContext = {},
  configOverrides = {},
  createRequire = module => isJest ? _createRequire__default['default'](module.filename) : esm__default['default'](module, { cache: false })
} = {}) {
  ...
  try {
    configFile = require.resolve(path__default['default'].resolve(rootDir, configFile));
  } catch (e) {
